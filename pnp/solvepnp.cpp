...
float SolveP4p::getBuffPitch(float dist, float tvec_y, float ballet_speed)
{
    ...
    // 重力补偿枪口抬升角度
    float a = 0.0;
    float GRAVITY = 9000.7f; //shenzhen 9.7887  zhuhai//15 7500
    y_temp = tvec_y;
    // 迭代求抬升高度
    for (int i = 0; i < 20; i++)
    {
        ...
        // 计算实际落点
        float t;
        t = dist / ballet_speed /* * 180 / CV_PI */ * cos(a);
        y_actual = ballet_speed /* * 180 / CV_PI */ * sin(a)* t - GRAVITY * t * t / 2;
        dy = tvec_y - y_actual;
        y_temp = y_temp + dy;
        // cout<<"i ="<<i<<endl;
        // 当枪口抬升角度与实际落点误差较小时退出
 ...
 * @param pos_in_ptz 
 */
void SolveP4p::get_Angle(const Mat &pos_in_ptz, int bullet_speed)
{
    //计算子弹下坠补偿
    const double *_xyz = (const double *)pos_in_ptz.data;
    //cout << "x:" << _xyz[0] << "   y:" << _xyz[1] << "   z:" << _xyz[2] << endl;
    ...
    if (bullet_speed > 10e-3)
    {
        down_t = _xyz[2] / (bullet_speed*1000);
    }
    ...
    // double xyz[3] = {xyz[0], xyz[1] - offset_gravity, xyz[2]};
    /*------------------------------------北理珠---------------------------------------------------*/
    dist = static_cast<float>(xyz[2]);
    angle_x = static_cast<float>(angle_x) * 180 / CV_PI;
    float target_h = ROBOT_H + ARMOR_BOTTOM_H;
    // float thta = -static_cast<float>(atan2(xyz[1], xyz[2]));          // 云台与目标点的相对角度
    // cout<<"thta = "<<thta<<endl;
    // float balta = static_cast<float>(atan2(target_h, xyz[2])) + thta; // 云台与地面的相对角度
    // cout<<"balta = "<<balta<<endl;
    // dist = static_cast<float>(xyz[2]);

    
    // angle_y += pow(balta, 1+_xyz[2]/10000);//15
    
    
    // cout<<"angle_y = "<<angle_y<<endl;
        // angle_y += static_cast<float>(atan2(xyz[1]*0.001, xyz[2]*0.001));
    /*------------------------------------北理珠---------------------------------------------------*/
    ...
    angle_y -= getBuffPitch(xyz[2], target_h, bullet_speed*1000);

    // float _y = static_cast<float>(angle_y) * 180 / CV_PI;
//     cout<<"_xyz[1] = "<<_xyz[1]<<endl;
//     float thta  = static_cast<float>(atan2((ROBOT_H +_xyz[1])/1000, this->dist/1000));
//     // cout<<"thta = "<<thta<<endl;
//     // float balta = static_cast<float>(atan2((ROBOT_H +_xyz[1])/1000, this->dist/1000)) - thta;
//     // cout<<"balta = "<<balta<<endl;
//     // 申明临时y轴方向长度,子弹实际落点，实际落点与击打点三个变量不断更新（mm）
//     float balta = static_cast<float>(atan2(_xyz[1]/1000, this->dist/1000)) - fabs(thta);
//     cout<<"balta = "<<balta<<endl;
//     float y_temp, y_actual, dy;
//     // 重力补偿枪口抬升角度
//     float a       = balta+_y;
...
//     {
//         // 计算枪口抬升角度
//         angle_y = (float)atan2(y_temp, dist/1000);
//         // 计算实际落点
//         t=0.f;
// /*         #ifdef DEBUG
//         t = (float)((exp(init_k_ * x) - 1) / (init_k_ * v * cos(angle)));
//         #else */
//         t                 = dist / ((_bullet_velocity * cos(a))*1000);
//         // #endif // DEBUG
        
// /*         #ifdef DEBUG
//         y_actual = (tan(theta)+(m*g)/(k*v*cos(theta)))*x + (m*m*g)/(k*k)*log(1-(k*x)/(m*v*cos(theta)));//TODO:记得替换 theta k m x
//         #else */
//         y_actual          = _bullet_velocity * sin(a) * t - GRAVITY * t * t / 2;
//         // #endif // DEBUG
//         dy                = _xyz[1]*0.001 - y_actual;
//         y_temp            = y_temp + dy;
//         // 当枪口抬升角度与实际落点误差较小时退出 TODO:可调节
//         if (fabsf(dy) < 0.01) {
//             break;
//         }
//     }
